...
$mysql_path -D$local_db_name -u$local_db_username -p$local_db_password --host=127.0.0.1 --port=$local_db_port -Bse "DROP DATABASE IF EXISTS $local_db_name; CREATE DATABASE $local_db_name;"

ssh $ssh_string "mysqldump $remote_db_name --quote-names --opt --hex-blob --add-drop-database -h$remote_db_host -u$remote_db_username -p\"$remote_db_password\"" | $mysql_path -D$local_db_name -u$local_db_username -p$local_db_password --host=127.0.0.1 --port=$local_db_port
